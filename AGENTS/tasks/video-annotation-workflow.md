...
- The service does not use ML/detectors in v1. Automation is FFmpeg extraction plus keyframe-seeded rules-based tracking between accepted keyframes.
...
4. Accepted manual keyframes with matching `track_id` enqueue linear tracking proposals with confidence and trajectory warnings.
5. Autotracking creates generated proposals for target frames and puts them into review, not export.
...
8. Native JSON/JSONL preserve video/frame provenance, track ids, source (`manual`/`generated`/`no_object`), tracking confidence and trajectory warnings.
...
- `apps/labeling/jobs.py` - extraction and tracking/interpolation jobs.
- `apps/labeling/tracking.py` - rules-based tracking proposals, confidence and warning heuristics.
...
- Manual correction remains the escape hatch for tracking mistakes: reject generated proposals into `generated_rejected`/manual assignment instead of trying to silently fix identity.
