...
- Assignment quota - это эффективный лимит на активную работу пользователя в текущем раунде комнаты: персональный `RoomAssignmentQuota` override, иначе `Room.default_assignment_quota`. Отклонённые раунды возвращают квоту, потому что перестают быть current round; skipped work квоту не расходует.
