...
- Квота разметчика внутри комнаты считается от эффективного лимита: `RoomAssignmentQuota.task_quota`, если задан персональный override, иначе `Room.default_assignment_quota`. Отсутствие персонального override больше не означает unlimited, если у комнаты есть стандартная квота. Квоту расходуют только assignments текущего раунда в статусах `in_progress`/`submitted`; skipped assignments и старые отклонённые раунды квоту не занимают.
