...
- ?????? ???? ? ??????? ?? ID+?????? ????? ?? UI/API. ???? ??????? ??? ????? ?????????? - invite link / join request; ?????? ?????? ?????????? ?????? ??? ????????? ???????????? ???????. `RoomJoinView` ??? ???? ???????? explicit join endpoint-?? ??? ??? ??????? ??????, ? ?????? ??????? ??? ??? ?????????? ??? ????????? create/edit ? ???????? ?????? join service.
- Production deploy ?????? ???????? ? `pull_request_target.closed` ??? merged PR ? `main`, ? ?? ? ???????? `push` ? `main`: ??? ???????? ?????????? ?? ???????, ????? GitHub ?? ??????? push-triggered Actions run ????? merge. ?????? fallback ???????? ????? `workflow_dispatch`.
- ? main ??? ???? MVP ??????????? ????????????? ????? `VideoSelection`/`FrameAnnotationTask`; ????? ????? ????????? ??? frame-based video workflow ????? `VideoAsset`/`VideoFrame`, FFmpeg/RQ extraction, keyframe/interpolation-target states ? generated review proposals.
...
- [tasks/platform-redesign-rollout.md](tasks/platform-redesign-rollout.md)
  UI-????: ???????? ?????????????? ????? Refero/Seline-inspired dashboard style ? ??????? ?? ???????, ?????, room detail ? ??????? ????????? ??? ????????? ? fullscreen/editor UX.
...
- ????????? room default quotas, ???????????? quota overrides, skip exposure semantics ? rescue-pass ??? ???????? rejected/skip ????????? assignment-?.
- Image rooms ?????? ???????????? post-create dataset management: ?????????? ????????? ???????????/ZIP ? ???????? primary task rows ????? room dataset API.
- ???????? MVP ??????????? ?????????????: pre-annotation ????? ??? ?????? ??????/??????????, ????????? ?????????? frame tasks, ?????? SVG bbox-editor ?? ??????, ??????? `empty`/`uncertain` ? JSON-??????? ?????????? ?????????.
- ??????? ???????? ?????????? ?? ????? dashboard-style visual direction ? ????? ?????? ? ????????? ???????? ?????? ????? preset query parameters.
- ????? ???????? ??????? ????????? ?? progressive wizard: ????????, ????????, ??????, ???????, ???????? ???????? ? ????????? ????????????? ????? ?????????; video workflow ????????? ???? ????????? extraction/keyframe.
