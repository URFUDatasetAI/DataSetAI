...
function renderActivity(container, series) {
    ...
    const calendarWidth = targetWeekCount * ACTIVITY_CELL_SIZE + (targetWeekCount - 1) * ACTIVITY_CELL_GAP + ACTIVITY_CALENDAR_PADDING;
    ...
    <div class="activity-board__calendar" style="width: ${calendarWidth}px; max-width: 100%;">
      <div class="activity-board__months" style="grid-template-columns: repeat(${targetWeekCount}, ${ACTIVITY_CELL_SIZE}px);">
        ${monthLabels
        .map((item) => `<span class="activity-board__month" style="grid-column: ${item.weekIndex + 1};">${item.label}</span>`)
        .join("")}
      </div>
        ...
        grid.querySelectorAll("[data-room-pin-id]").forEach((button) => {
            button.addEventListener("click", async (event) => {
                ...
                try {
                    ...
                    showFlash(nextPinnedState ? `Комната #${roomId} закреплена.` : `Комната #${roomId} откреплена.`, "success");
                    await loadRooms();
                }
                catch (error) {
...
function initProfilePage() {
    ...
    const activity = byId("profile-activity");
    ...
    state.pageRefresh = async () => {
        ...
        try {
            const profile = await api(profileUserId === state.user.id ? "/api/v1/me/profile/" : `/api/v1/users/${profileUserId}/profile/`);
...
function initRoomCreatePage() {
    ...
    const testTaskCountField = byId("test-task-count-field");
    const labelEditorSection = byId("label-editor-section");
    const labelEditorList = byId("label-editor-list");
    const addLabelBtn = byId("add-label-btn");
    const crossValidationToggle = byId("cross-validation-enabled");
    const crossValidationCountInput = byId("cross-validation-count");
    const crossValidationThresholdInput = byId("cross-validation-threshold");
    ...
    form?.addEventListener("submit", async (event) => {
        ...
        payload.append("cross_validation_annotators_count", String(Number(rawFormData.get("cross_validation_annotators_count") || 1)));
        payload.append("cross_validation_similarity_threshold", String(Number(rawFormData.get("cross_validation_similarity_threshold") || 80)));
...
function initRoomEditPage() {
    ...
    const form = byId("room-edit-form");
    const titleInput = byId("room-edit-title");
    const descriptionInput = byId("room-edit-description");
    const datasetLabelInput = byId("room-edit-dataset-label");
    const deadlineInput = byId("room-edit-deadline");
    const passwordToggle = byId("room-edit-password-enabled");
    const passwordInput = byId("room-edit-password");
    const passwordNote = byId("room-edit-password-note");
    const readonlySummary = byId("room-edit-readonly-summary");
...
function getReviewPayloadAnnotations(payload) {
    ...
    return payload.annotations.filter((annotation) => annotation &&
...
function renderReviewComparison(consensusContainer, annotationsContainer, comparisonSection, rejectBtn, detail) {
...
function renderRoomDashboardHeader(title, subtitle, roomHeaderMeta, roomMetrics, dashboard) {
    ...
    if (dashboard.actor.can_edit_room) {
        roomActions.push(`<a class="btn btn--muted" href="/rooms/${dashboard.room.id}/edit/">Редактировать комнату</a>`);
    }
    if (dashboard.actor.can_delete_room) {
        roomActions.push('<button id="detail-delete-room-btn" class="btn btn--danger" type="button">Удалить комнату</button>');
    }
...
function initRoomDetailPage() {
    ...
    const title = byId("room-title");
    const subtitle = byId("room-subtitle");
    const roomHeaderMeta = byId("room-header-meta");
    const roomMetrics = byId("room-metrics");
    const annotatorWorkspace = byId("annotator-workspace");
    const annotatorManageSide = byId("annotator-manage-side");
    const customerWorkspace = byId("customer-workspace");
    const annotatorSummary = byId("annotator-summary");
    const annotatorActivity = byId("annotator-activity");
    const workBtn = byId("detail-work-btn");
    const workNote = byId("detail-work-note");
    const annotatorsList = byId("annotators-list");
    const annotatorDetailPanel = byId("annotator-detail-panel");
    const annotatorDetailActivity = byId("annotator-detail-activity");
    const inviteForm = byId("detail-invite-form");
    const inviteUserIdInput = byId("detail-invite-user-id");
    const exportPanel = byId("detail-export-panel");
    const invitePanel = byId("detail-invite-panel");
    const customerLabels = byId("customer-labels");
    const exportFormatSelect = byId("detail-export-format");
    const exportBtn = byId("detail-export-btn");
    const reviewTasksList = byId("review-tasks-list");
    const reviewTaskDetail = byId("review-task-detail");
    const reviewComparisonSection = byId("review-comparison-section");
    const reviewConsensusDetail = byId("review-consensus-detail");
    const reviewAnnotationsDetail = byId("review-annotations-detail");
    const rejectReviewTaskBtn = byId("reject-review-task-btn");
    const annotatorsSearchInput = byId("detail-annotators-search");
    const reviewSearchInput = byId("detail-review-search");
    function normalizeSearchValue(value) {
        return String(value || "").trim().toLowerCase();
    }
    function bindDeleteRoomButton() {
        const deleteRoomBtn = byId("detail-delete-room-btn");
        deleteRoomBtn?.addEventListener("click", async () => {
            clearFlash();
            const shouldDelete = window.confirm("Удалить комнату? Это действие удалит саму комнату, задачи, участников и результаты разметки без возможности восстановления.");
            if (!shouldDelete) {
                return;
            }
            try {
    ...
    function bindAnnotatorRoleControls(dashboard, annotator) {
        const roleSelect = byId("annotator-role-select");
        const roleSubmit = byId("annotator-role-submit");
    ...
    async function loadReviewTaskDetail(taskId) {
        ...
        renderReviewComparison(reviewConsensusDetail, reviewAnnotationsDetail, reviewComparisonSection, rejectReviewTaskBtn, detail);
    }
    async function loadReviewTasks() {
        ...
        const filteredTasks = state.reviewTasks.filter((task) => {
            const matchesAnnotator = !state.selectedAnnotatorUserId ||
        ...
        if (!state.reviewTasks.length) {
            ...
            renderReviewComparison(reviewConsensusDetail, reviewAnnotationsDetail, reviewComparisonSection, rejectReviewTaskBtn, null);
            return;
        }
        ...
        `)
            .join("");
        const selectedTaskId = filteredTasks.find((item) => item.id === state.selectedReviewTaskId)?.id ?? filteredTasks[0].id;
    ...
    rejectReviewTaskBtn?.addEventListener("click", async () => {
        ...
        clearFlash();
        const shouldReject = window.confirm("Отклонить эту разметку? Задача будет исключена из итоговой выборки и отправлена на повторную разметку.");
        if (!shouldReject) {
            return;
        }
        try {
...
function initRoomWorkPage() {
    ...
    const joinBtn = byId("work-join-btn");
    const topSubmitBtn = byId("work-submit-btn-top");
    const taskBox = byId("work-task-box");
    const submitForm = byId("work-submit-form");
    const resultJson = byId("work-result-json");
    const resultLabel = byId("work-result-label");
    const submitBtn = submitForm?.querySelector('button[type="submit"]');
    const mediaEditor = createMediaAnnotationEditor({
        mediaTool: byId("work-media-tool"),
        instructions: byId("work-media-instructions"),
        labelPalette: byId("work-label-palette"),
        activeLabelNote: byId("work-active-label"),
        mediaStage: byId("work-media-stage"),
        annotationList: byId("work-annotation-list"),
        clearBtn: byId("work-clear-annotations-btn"),
        resultJson,
        resultLabel,
        submitBtn,
    });
