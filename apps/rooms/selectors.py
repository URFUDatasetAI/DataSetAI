...
def build_room_invite_preview(*, room: Room, actor: User | None, request=None) -> dict:
    ...
    return {
        "room": {
            ...
            "description_pdf_url": request.build_absolute_uri(room.description_pdf.url) if request and room.description_pdf else (room.description_pdf.url if room.description_pdf else None),
            "description_pdf_name": room.description_pdf.name.rsplit("/", 1)[-1] if room.description_pdf else "",
