...
После обновления проекта миграции обязательны, потому что у комнаты появились поля пароля, дедлайна и названия датасета.
...
- Комнаты: `http://127.0.0.1:8000/rooms/`
- Профиль: `http://127.0.0.1:8000/profile/`
- Вход: `http://127.0.0.1:8000/auth/login/`
- Регистрация: `http://127.0.0.1:8000/auth/register/`
...
Во встроенном интерфейсе теперь есть несколько страниц.

Что умеет:

- главная страница с описанием проекта
- навигация в хедере: главная, комнаты, профиль
- кнопки `Войти / Зарегистрироваться` для анонимного пользователя
- кнопки `Профиль / Выйти` для авторизованного пользователя
- страница комнат с квадратными карточками доступных пользователю комнат
- прямой вход в комнату по `id + пароль`
- страница создания комнаты для заказчика
- рабочая страница комнаты со статистикой и активностью
- профиль пользователя с тепловой картой активности

Тестовые аккаунты после `seed_mvp_data`:

- `admin / admin` — заказчик
- `user / user` — разметчик
...
3. Выбери пользователя в хедере.
4. Перейди на страницу комнат.
5. Под заказчиком создай комнату или открой существующую.
6. Под разметчиком открой доступную комнату, возьми задачу и отправь результат.
