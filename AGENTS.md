...
- Cross-validation теперь поддерживает deterministic grouped distribution: при достаточном числе исполнителей задачи распределяются по фиксированным группам ревьюеров, а не только по отдельным annotator assignment-ам.
...
- Создатель комнаты больше не обязан быть исполнителем: это регулируется флагом `Room.owner_is_annotator`, который влияет и на policy доступа, и на dashboard/review payload-ы.
...
- Сохранять корректность нового grouped cross-validation flow: deterministic reviewer groups, fallback на legacy distribution и repeat-round semantics не должны деградировать.
...
- Не ломать owner-role semantics в комнатах: `owner_is_annotator` должен согласованно отражаться в create/update flow, policies, выдаче задач и review/dashboard представлениях.
...
- Для grouped cross-validation важна детерминированность: одна и та же задача должна назначаться одной и той же reviewer-group там, где room можно разбить на полные группы нужного размера.
...
- Роль владельца комнаты не фиксирована как annotator по умолчанию: `owner_is_annotator` влияет на can_annotate policy, eligible annotator pool, статистику участников и review payload-ы.
...
- `apps/labeling/distribution.py`
  - Deterministic выбор designated annotators, grouped cross-validation distribution и fallback на legacy balanced assignment.
...
- `apps/labeling/tests.py` и `apps/rooms/tests.py`
  - Актуальная спецификация grouped cross-validation, consensus, owner role semantics, room create/update flow и review/dashboard payload-ов.
- `tests/test_rooms_api.py` и `tests/test_labeling_api.py`
  - Дополнительная интеграционная спецификация room/access flows, detect+text, review и export.
...
- Для grouped cross-validation `apps.labeling.distribution` задаёт deterministic reviewer groups по `item_number`/task ordering; если пул annotator-ов нельзя разбить на полные группы нужного размера, система обязана fallback-нуться на legacy balanced-per-annotator strategy, а не молча ломать назначение.
...
- Повторные раунды grouped cross-validation не должны произвольно менять reviewer-group для задачи: если задача уже ушла в следующий раунд из-за расхождения, designated pool для неё должен оставаться согласованным с действующей distribution logic.
...
- `Room.owner_is_annotator` должен консистентно влиять на create/update serializers, `can_annotate_room`, eligible annotator pool в labeling services и состав участников/статистики в room selectors.
