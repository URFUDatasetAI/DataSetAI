...
- Экспорт (`export_room_annotations`): Поддерживает Native JSON, JSONL, COCO, YOLO и Pascal VOC. Динамически собирает архив по разметкам, которые перешли в статус `SUBMITTED`.
